...
try {
    ...
    // 6️⃣ Check if result exists
    $stmt = $conn->prepare("SELECT result_id FROM stud_quiz_results WHERE quiz_id = :quiz_id AND stud_id = :stud_id");
    $stmt->execute(['quiz_id' => $quiz_id, 'stud_id' => $student_id]);
    $existing = $stmt->fetchColumn();

    if ($existing) {
        // 7️⃣ Update existing result
        $stmt = $conn->prepare("
            UPDATE stud_quiz_results
