...
<div class="header">
    <div class="header-left">
      <img src="/CAPSTONE_LMS_EHS/assets/landingpage/ehslogo.png" class="header-logo" alt="Logo">
  ...
  <div class="header-buttons">
    ...
    <button class="action-btn" id="backBtn">⬅ Back to Exams</button>
  </div>
</div>
...
<div class="container">
  ...
  <ul>
    <li>Use <strong>Create Question</strong> to add new questions to the questionnaire.</li>
    <li>You may create <strong>Multiple Choice</strong>, <strong>Checkbox</strong>, or <strong>Short Answer</strong> questions.</li>
    <li>Mark at least one option as the <strong>correct answer</strong> for auto-checking.</li>
    <li>Click <strong>Publish</strong> to assign this exam to a specific Grade, Strand, and Section.</li>
    <li>After students submit their answers, use <strong>Responses</strong> to view scores and submissions.</li>
    <li>You can <strong>Edit</strong> or <strong>Delete</strong> student responses anytime.</li>
  </ul>
</div>
...
<div class="modal" id="publishModal">
  <div class="modal-content">
    ...
    <form id="publishForm">
      ...
      <textarea name="description" id="examDesc" rows="4"></textarea>
      ...
      <div class="modal-footer">
        <button type="submit" class="save-btn" id="publishExamBtn">Publish</button>
      </div>
    </form>
  </div>
</div>
...
$(document).ready(function() {
  ...
  function loadQuestions() {
    $.ajax({
      url: '/CAPSTONE_LMS_EHS/api/prof/prof_exam/get_exam.php',
      type: 'GET',
      data: { exam_id: examId },
      dataType: 'json',
      success: function(data) {
        ...
        data.forEach(q => {
          ...
          const newQuestion = $(`<div class="question-card" data-question-id="${q.question_id}">
                                    <button class="delete-btn" data-id="${q.question_id}">
  ...
  $('#saveQuestion').click(function() {
    ...
    $.ajax({
      url: '/CAPSTONE_LMS_EHS/api/prof/prof_exam/add_question.php',
      type: 'POST',
      data: {
        exam_id: examId,
        question_type: type,
  ...
  $(document).on('click', '.question-card .delete-btn', function() {
    ...
    $.ajax({
      url: '/CAPSTONE_LMS_EHS/api/prof/prof_exam/delete_question.php',
      type: 'POST',
  ...
  $('#publishForm').submit(function(e) {
    ...
    const description = $('#examDesc').val().trim();
    ...
    $.ajax({
      url: '/CAPSTONE_LMS_EHS/api/prof/prof_exam/add_exam.php',
      type: 'POST',
      data: {
        exam_id: examId,
...
function loadResponses() {
    $.ajax({
      url: '/CAPSTONE_LMS_EHS/api/prof/prof_exam/get_responses_exam.php',
      type: 'GET',
      data: { exam_id: examId },
      dataType: 'json',
      success: function(data) {
        ...
        data.forEach(r => {
          ...
          tbody.append(`
            <tr>
              ...
              <td>${score} / ${totalPoints}</td> <!-- perfect score displayed here -->
              <td>${new Date(r.submitted_at).toLocaleString()}</td>
              <td>
                <button class="view-btn" data-result-id="${r.result_id}">View</button>
                <button class="edit-btn" data-result-id="${r.result_id}" data-score="${score}" data-max="${totalPoints}">Edit</button>
              </td>
            </tr>
          `);
        });
...
$(document).on('click', '.edit-btn', function() {
    ...
    $.ajax({
      url: '/CAPSTONE_LMS_EHS/api/prof/prof_exam/update_score.php',
      type: 'POST',
  ...
  $(document).on('click', '.view-btn', function() {
    ...
    window.open(
        `/CAPSTONE_LMS_EHS/profdashboard/openreq/exam_results_view.php?result_id=${resultId}`,
        '_blank',
  ...
  $('#backBtn').click(function() {
    window.location.href = '/CAPSTONE_LMS_EHS/profdashboard/openreq/exam.php';
  });
});
</script>
