...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_module'])) {
    ...
    $filePath = "$prof_name/{$strand}-{$section}/module/$fileName";

    $supabaseUrl = 'https://fgsohkazfoskhxhndogu.supabase.co';
    $bucket      = 'teacher_module_file';
    $anonKey     = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZnc29oa2F6Zm9za2h4aG5kb2d1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjA0MzU4MDIsImV4cCI6MjA3NjAxMTgwMn0.EHpoxrGBEx9j2MYQPbhGo-l65hmfijmBBRY65xMVY7c'; // must match your policy
    ...
    $sql = "INSERT INTO prof_modules 
        (teacher_id, class_id, section, title, date_posted, \"desc\", file_name, file_path)
        VALUES (:teacher_id, :class_id, :section, :title, :date_posted, :desc, :file_name, :file_path)";
...
if (isset($_GET['delete_id'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM prof_modules WHERE module_id = :id");
...
// Edit module
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_module_id'])) {
    $edit_id = intval($_POST['edit_module_id']);
    $edit_title = $_POST['edit_title'];
    $edit_desc = $_POST['edit_desc'];

    $stmt = $conn->prepare("UPDATE prof_modules 
...
// Fetch modules
$stmtFetch = $conn->prepare("SELECT * FROM prof_modules WHERE teacher_id = :teacher_id AND class_id = :class_id AND section = :section ORDER BY module_id DESC");
$stmtFetch->execute(['teacher_id' => $teacher_id, 'class_id' => $class_id, 'section' => $section]);
$modules = $stmtFetch->fetchAll(PDO::FETCH_ASSOC);


// Forward module
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['forward_module_id'])) {
    $forward_id = intval($_POST['forward_module_id']);
    $target_class = intval($_POST['forward_class_id']);

    // Fetch current module details
    $stmt = $conn->prepare("SELECT * FROM prof_modules WHERE module_id = :module_id");
    $stmt->execute(['module_id' => $forward_id]);
    $module = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($module) {
        // Get section from target class
        ...
        // Insert a copy into the target class
        $stmtInsert = $conn->prepare("
            INSERT INTO prof_modules 
            (teacher_id, class_id, section, title, date_posted, \"desc\", file_name, file_path)
            VALUES 
            (:teacher_id, :class_id, :section, :title, :date_posted, :desc, :file_name, :file_path)
        ");
...
<div class="main">
  ...
  <div class="filter-buttons">
    ...
    <a href="/CAPSTONE_LMS_EHS/profdashboard/openreq/module.php?class_id=<?= $class_id ?>&section=<?= $section ?>&strand=<?= $strand ?>" class="filter-btn active"><span class="material-icons">menu_book</span>Module</a>
  ...
  <div class="task-list">
    <!-- New Module Card -->
    <div class="task-card new-task" id="newModuleBtn">
      <div class="task-icon"><span class="material-icons">add_circle</span></div>
      <div class="task-content">
        <h4>Upload New Module</h4>
        <p>Click the plus icon to upload a new module for this course.</p>
      </div>
    </div>
    ...
    <div id="moduleModal" class="modal">
      <div class="modal-content">
        ...
        <form method="POST" enctype="multipart/form-data">
          ...
          <textarea name="desc" rows="4" placeholder="Enter module details" required></textarea>
          <label>Attach File:</label>
          <input type="file" name="file" accept=".pdf,.doc,.docx,.ppt,.pptx,.jpg,.png" required>
...
<div id="forwardModal" class="modal">
  <div class="modal-content">
    ...
    <form method="POST" id="forwardForm">
      <input type="hidden" name="forward_module_id" id="forward_module_id">
    ...
    <?php foreach ($modules as $module): ?>
      <div class="task-card">
        ...
        <div class="task-content">
          <h4><?php echo htmlspecialchars($module['title']); ?><span class="task-tag">Module</span></h4>
          <p><?php echo nl2br(htmlspecialchars($module['desc'])); ?></p>
          <p>Date Uploaded: <?php echo htmlspecialchars($module['date_posted']); ?></p>
          <?php if (!empty($module['file_path'])): ?>
            <div class="file-overview">
              <a href="#" class="file-name" data-file="<?php echo htmlspecialchars($module['file_path']); ?>">
        ...
        <div class="task-meta">
          ...
          <div class="dropdown-menu">
            <a href="#" onclick="openEditModal(<?php echo htmlspecialchars(json_encode($module)); ?>)">Edit</a>
            <a href="#" onclick="openForwardModal(<?php echo htmlspecialchars(json_encode($module)); ?>)">Forward</a>
            <a href="?delete_id=<?php echo $module['module_id']; ?>" onclick="return confirm('Delete this module?')">Delete</a>
          </div>
        </div>
      </div>
    <?php endforeach; ?>
...
const openBtn = document.getElementById("newModuleBtn");
...
function openEditModal(module) {
    ...
    document.getElementById("edit_desc").value = module.desc;
...
function openForwardModal(module) {
    document.getElementById("forward_module_id").value = module.module_id;
