...
$title = $_GET['title'] ?? 'Attendance';
$strand = $_GET['strand'] ?? '';
$section = $_GET['section'] ?? '';

// Fetch teacher name
$teacher_name = 'Unknown_Teacher';
$stmt = $conn->prepare("SELECT first_name, last_name FROM teachers_account WHERE teacher_id = ?");
...
$results = [];
$stmt = $conn->prepare("
    SELECT sa.stud_id, s.first_name, s.last_name, sa.status, sa.submission_time
    FROM student_attendance sa
    JOIN students_account s ON sa.stud_id = s.stud_id
...
<div class="main">
    <div class="topbar">
    <a href="attendance.php?course=<?= urlencode($_SESSION['course'] ?? '') ?>&strand=<?= urlencode($strand) ?>&section=<?= urlencode($section) ?>" class="back-btn">
        <span class="material-icons">arrow_back</span>
    </a>
    Attendance Records - <?= htmlspecialchars($strand) ?> - <?= htmlspecialchars($section) ?>
</div>
            ...
            <?php else: ?>
                <?php foreach ($results as $res): ?>
                    <tr>
                        ...
                        <td><?= (new DateTime($res['submission_time']))->format('Y-m-d h:i A') ?></td>
                    </tr>
                <?php endforeach; ?>
            <?php endif; ?>
        </tbody>
