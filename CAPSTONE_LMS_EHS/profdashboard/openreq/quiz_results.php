...
// Fetch quiz results for this teacher
$results = [];
$stmt = $conn->prepare("
    SELECT s.first_name, s.last_name, q.title, r.score, r.date_taken
    FROM stud_quiz_results r
    JOIN prof_quiz q ON r.quiz_id = q.quiz_id
...
<div class="main">
    <div class="topbar">
        <a href="quizzes.php?course=<?= urlencode($course_name) ?>&strand=<?= urlencode($strand) ?>&section=<?= urlencode($section) ?>" class="back-btn">
            <span class="material-icons">arrow_back</span>
        </a>
        Quiz Results (<?= htmlspecialchars($strand) ?> - <?= htmlspecialchars($section) ?>)
    </div>
    ...
    <table>
        ...
        <tbody>
            <?php if (empty($results)): ?>
                <tr><td colspan="4">No quiz results found.</td></tr>
            <?php else: ?>
